...
%selpath = uigetdir; % choose your directory / folder of your data.
%%
...
darkfile = fullfile('..\',append(data_date,'_BRDF_Albert'),'*_DRK_SP_NaN*.txt');%fullfile('..\',append(data_date,'_BRDF_Albert'),'*_DRK_SP_*.txt'); %fullfile('*_DRK_*.txt'); %
...
white_data = white_data([9:-1:6,1,2:5]); %white_data([9:-1:6,1,2:5]); %select correct order of measurements
...
hold on
for idx = 1:size(white_measurements,2)
plot(white_rel_sensor_ang(:,idx),f_ellipse(:,idx),'.-','MarkerSize',5)
end
hold off
grid on
grid minor
xlim([-90,90])
xlabel('view angle $\alpha$')
ylabel('area correction factor')
lgd = legend(strcat(num2str(white_sample_ang(1,5:-1:1)','%d'),'$^\circ$'),'Location','eastoutside');
title(lgd,'Sample Angle')

fig = figure;
plot(white_rel_sensor_ang(2:end-1,5),f_sensorang(2:end-1),'k.--','MarkerSize',5)
grid on
grid minor
xlim([-90,90])
xlabel('view angle $\alpha$')
ylabel('view angle correction')

fig = figure;
plot(white_sample_ang(19,:),f_sampleang,'k.--','MarkerSize',5)
grid on
grid minor
xlim([-90,90])
xlabel('incidence angle $\theta$')
xtickformat('%d')
ylabel('incidence angle correction')

%% import sample data
% import only data with '.' as decimal separator
% run sed_gui.py or sed_comma_period.py on files before importing
% select files to import using wildcards
sample_files =dir(fullfile('..\',append(data_date,'_BRDF_Albert'),'*_SP_COUNTS_Samp_*.txt')); %dir(fullfile('..\*_BRDF_Albert','*_Lenz_*.txt'));
...
%{
Wol_corr = dir(fullfile('..\',append(data_date,'_BRDF_Albert'),'*_Wolbringrest_*.txt'));
W_corr_data = importReflectance_separate(fullfile({Wol_corr.folder}',{Wol_corr.name}'),darkfile,calibration_path,'singledark',true);
% Correction of the broken data point in Wolbring.
% File number 15. Six last measurements.

for i=1:6
   sample_data(15).measurements(31+i).counts = W_corr_data(1).measurements(i).counts;
   sample_data(15).measurements(31+i).raw = W_corr_data(1).measurements(i).raw;
   sample_data(15).measurements(31+i).spectral_flux = W_corr_data(1).measurements(i).spectral_flux;
end
%W_corr_data(1).measurements(3).counts
%(sample_data(15).measurements(32:end).counts)
%}
...
%var_ang = zeros(21,size(f_ellipse,2));
var_ang = [-90:5:90]; %[-10:1:10;5:1:25;20:1:40;35:1:55;50:1:70]; % ;5:1:25;20:1:40;35:1:55;50:1:70
for idx = 1:size(f_ellipse,2)
    f_ellipse(:,idx) = calcOverlapFactor_v2(ones(37,1).*f_ellipse_angles(idx),[-90:5:90]','sensorspot_overlap_small.mat'); %calcOverlapFactor_v2(ones(37,1).*f_ellipse_angles(idx),[-90:5:90]','sensorspot_overlap_small.mat');
end
...
BRDF_names = repmat(reshape({BRDF_info.SAMPLE_NUMBER},1,nsamples,[]),21,1,1);
...
%% Calculation of reflectance of silicon under assumption of completely diffuse.
for idx3 = 1:size(BRDF_measurements,3)
    for idx2 = 1:size(BRDF_measurements,2)
 
 BRDF_irradiance(:,idx2,idx3) = sample_radiant_flux./app_area(white_rel_sensor_ang==f_ellipse_angles(idx2)&white_sample_ang==f_ellipse_angles(idx2)); %BRDF_meas_radiant_flux(:,idx2,idx3)./app_area(white_rel_sensor_ang==f_ellipse_angles(idx2)&white_sample_ang==f_ellipse_angles(idx2));
 BRDF_exitance(:,idx2,idx3) = 2*pi*trapz(deg2rad(BRDF_rel_sensor_ang(1:ceil(size(BRDF_rel_sensor_ang,1)/2),idx2,idx3)),[BRDF_radiance(1:ceil(size(BRDF_rel_sensor_ang,1)/2),idx2,idx3)].*abs(sind(2.*BRDF_rel_sensor_ang(1:ceil(size(BRDF_rel_sensor_ang,1)/2),1))/2)',2);

    end
      mean(BRDF_exitance((2:end),idx2,7));
end
R_lambda = ((BRDF_spectral_exitance_0ang(:,8)) ./ sample_spectral_irradiance(:,2));
R = mean(R_lambda(~isnan(R_lambda)))
size(BRDF_exitance);
size(BRDF_irradiance);
%BRDF_Reflectance = BRDF_exitance ./  BRDF_irradiance(1:19,:,:);%(2.0794*10^4);%sample_irradiance; %BRDF_irradiance(1:19,:,:);
    mean(BRDF_exitance(~isnan(BRDF_exitance(:,1,8)),1,8) ./ (BRDF_irradiance(1:18,5,8)),1)


for i=1:5
   % BRDF_irradiance(1:19,i,8)
BRDF_silicon(:,i) = mean(BRDF_exitance(~isnan(BRDF_exitance(:,i,8)),i,8) ./  BRDF_irradiance(1:numel(~isnan(BRDF_exitance(:,i,8)),i,8)),1)
%BRDF_silicon(:,i)
%mean(BRDF_silicon(~isnan(BRDF_silicon(:,i))),1);
end
...
%% D65 XYZ colorimetry plots


%p = inputParser;
%addParameter(p,'whitepoint_CCT',6504,@(x) validateattributes(x,{'numeric'},{'scalar'}));
%whitepoint_CCT = p.Results.whitepoint_CCT;
% generate whitepoint from D-illuminant for measurement wavelengths
%whitepoint_SPD = generateDilluminant(whitepoint_CCT,reflectance(:,1));

D65 = extractStructfieldCell(sample_colorimetry,'D65',size(sample_colorimetry,2),size(sample_colorimetry,3));
cmf = extractStructfieldCell(sample_colorimetry,'cmf',size(sample_colorimetry,2),size(sample_colorimetry,3));

b = D65{1,1,1};
d = cmf{1,1,1};
figure(10)
plot(wavelengths,b(:,2),'LineWidth',2,'Color','black')
xlim([300,800])
xlabel('Wavelength $\lambda$[nm]','FontSize',18)
ylabel('Rel. power [a.u]','Fontsize',18)
grid("on")

figure(11)
plot(wavelengths,d(:,1),'LineWidth',2,'Color','red')
hold on
plot(wavelengths,d(:,2),'LineWidth',2,'Color','blue')
plot(wavelengths,d(:,3),'LineWidth',2,'Color','green')
xlim([300,800])
xlabel('Wavelength $\lambda$[nm]','FontSize',18)
ylabel('Rel. power [a.u]','Fontsize',18)
grid("on")
legend({'$\bar{x}$','$\bar{y}$','$\bar{z}$'},'Interpreter','latex','FontSize',16)

%% Saving relevant data to matfile.
...
switch choice
    case 'Yes'
        ...
        PV_name = input(Prompt,"s");
        file_name = sample_files.name;
        file_name = file_name(1:10); % Find the date of the data file. 
        folder = 'C:\Users\Klugy\OneDrive - Danmarks Tekniske Universitet\DTU\5 semester - kandidat\Speciale\Experimental_data\Processed_data';
        if exist(folder,'dir') == 0 % Make Folder if it does not exist.
            mkdir(folder)
        end
        Full_file_name = fullfile(folder,append(file_name,'_',PV_name));
        save(Full_file_name,'BRDF_spectral_brdf','BRDF_brdf','f_ellipse_angles','white_spectral_brdf','white_rel_sensor_ang','white_meas_radiance','sample_Lab','sample_files','sample_rgb','wavelengths','white_sample_ang','white_brdf')
