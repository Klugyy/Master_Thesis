...
testFile = SampleDir.Concat(SampleDir,'PV_BSDF_analysis_tester.zmx'); % Lens file
%ZOS_system.New(testFile,false)
ZOS_system.LoadFile(testFile,false);
ZOS_instance.LoadNewSystem('PV_BSDF_analysis_tester.zmx')
LDE = ZOS_system.LDE;
%LDE.GetSurfaceAt(3)
Nce = ZOS_system.NCE;
...
%}
     incidence_array = linspace(0,60,60/3 +1); %[15]; %linspace(0,60,60/3 +1);
     ...
     P_Reflective = [];
     space = [];
     source_size =linspace(0.01,5,30);
     coordinates = cell(3,numel(incidence_array));
     col = cell(1,numel(source_size));
     for F= 1:numel(source_size)
      theta_i = 15;%incidence_array(1,F);
     ...
     ZOS_system.NCE.GetObjectAt(2).GetObjectCell(ZOSAPI.Editors.NCE.ObjectColumn.TiltY).DoubleValue
    ...
    NSCRayTrace.SaveRaysFile = 'Lambertian_glass_5000_v2.ZRD';%'One_Ray_No_scat.ZRD';
...
%}
    ...
    % read results file
        ...
        ZRD_reader.ZRDFile = 'C:\Users\s194086\Documents\Zemax\Samples\Lambertian_glass_5000_v2.ZRD'; % Ray_1000_const.ZRD % Lambertian_glass_1000.ZRD
       ...
       % ZRDResult.ReadNextResult
            ...
            while success_NextSegmentFull == 1
             ...
             %  if idx_azi ==0 
              ...
              % end
               ...
               preseg_cur = preseg_data(I,:); %  [preseg,hitobj,seglvl,j-1,sprintf("%f",segint)];
               hit_array = zeros(3,1); % zero if obj not hit, 1 if object is hit for the ray path.
               preseg_cur(1,1)
               if preseg_cur(1,1) == 0
                   hit_array = [1,0,0].';
               end
               q = numel(preseg_data(:,1));
             ...
             %  Ray_hit
               ...
               Hit_count_relf = Hit_count_relf + 1;
                ...
                Hit_count = Hit_count + 1;
                Data_d8(Hit_count,:) = {[rayNumber,hitobj,numSegments,seglvl,preseg],[Ang,Ang_pol,idx_azi],segint,[l,m,n,Nx,Ny,Nz]};
        ...
        [sorted_MMM, sortIdx] = sort(MMM(:));

        % Step 2: Sort MM using the sorted indices of MMM
        sorted_MM = MM(sortIdx);
           
        % Step 3: Find the maximum value in MM for each unique value in MMM
        unique_sorted_MMM = unique(sorted_MMM);
        div = [div,max(arrayfun(@(x) max(sorted_MM(sorted_MMM == x)) - min(sorted_MM(sorted_MMM == x)), unique_sorted_MMM))]; %max(arrayfun(@(x) max(sorted_MM(sorted_MMM == x)), unique_sorted_MMM));
        %[u_max,v_max] = max(MM);
        %[u_min,v_min] = min(MM);
        %div = [div,abs(u_max-u_min)];
        %div_max = [div_max,MMM(v_max)];
        %div_min = [div_min,MMM(v_min)];
...
set(gca, 'FontSize', 20)  % Increase tick size
xlabel('X [mm]',fontsize = 24)
ylabel('Y [mm]',fontsize = 24)
zlabel('Z [mm]',fontsize = 24)
hold off
legend({'Detector',sprintf(str_legend,'$r_{sc}$',source_size(10),'$\mathrm{mm}$'),sprintf(str_legend,'$r_{sc}$',source_size(20),'$\mathrm{mm}$'),sprintf(str_legend,'$r_{sc}$',source_size(end),'$\mathrm{mm}$')},'interpreter','latex','Location','best',Fontsize = 22)
...
zlabel('Z [mm]',fontsize = 24)
hold off
legend({'Detector',sprintf(str_legend,'$\theta_i$',incidence_array(1),'$^\circ$'),sprintf(str_legend,'$\theta_i$',incidence_array(11),'$^\circ$'),sprintf(str_legend,'$\theta_i$',incidence_array(end),'$^\circ$')},'interpreter','latex','Location','best',Fontsize = 22)
%}
    ...
    for n=1:numel(Azimuth_filter)
       ...
       % sum(val(2,1))
        ...
        if val(5,ii) == 3
            ...
            deg_diff = [deg_diff,(abs(val(3,1)-theta_i))-val(7,1)];
...
set(gca,'Layer','Top')
set(gca,'ThetaColor','blue','Rcolor','red','FontSize',16,'FontWeight','bold','LineWidth',3)
%pax.ThetaColor = 'blue';
%pax.RColor = [0 .5 0];
...
figure;
plot(Ray_hit(1:end-40),deg_diff(1:end-40),'Marker','square','Color','black','LineWidth',2,'LineStyle','none')
xlabel('Object hit',FontSize= 20)
ylabel('Polar Angle spread difference [Deg]',FontSize = 20)
grid('on')
%%

[unique_Ray_hit, ~, idx] = unique(Ray_hit(1:end-10));

% Initialize an array to store the mean values
mean_deg_diff = zeros(size(unique_Ray_hit(1:end-100)));
std_deg_diff = zeros(size(unique_Ray_hit(1:end-100)));


% Calculate the mean of deg_diff for each unique value in ray_hit
for i = 1:length(unique_Ray_hit)

    mean_deg_diff(i) = mean(abs(deg_diff(idx == i)));
    std_deg_diff(i) = std(abs(deg_diff(idx == i)));
end
%mean_deg_diff
%std_deg_diff
...
xlabel('Polar Angle $\theta_p [^\circ]$',fontsize = 24)
...
plot(source_size,div,'linewidth',2,'color','black','Marker','square','LineStyle','none')
grid('on')
xlabel('Source Radius $r_{sc}[\mathrm{mm}]$',fontsize = 20)
ylabel('$\mathrm{|\theta_{p,max}-\theta_{p,min}|} [^\circ]$',fontsize = 20)
set(gca,FontSize = 22)
xlim([0,5])
%}
%ratio =  div(1) ./ 0.75;
%%
%{
arce = circ/numel(Polar_filter);
bob = div'.*arce(1:3:63,1);
r_values = zeros(numel(bob),1);
for i=1:numel(bob)
x_val = coordinates{1,i};
y_val = coordinates{2,i};
[z_val,vups] = (sort(coordinates{3,i}));
ups = z_val(end-space(i));%max(z_val);
coord_max = [x_val(vups(end-space(i)-4)),y_val(vups(end-space(i)-4)),ups];
ups = z_val(1+space(i));
coord_min = [x_val(vups(4+space(i))),y_val(vups(4+space(i))),ups];

r_values(i,1) = norm(coord_max-coord_min);
%ud = [incidence_array(i)-div(i)/2,div(i)/2+incidence_array(i)];
%floor(ud(1))
%r_values(i,1) = sum(arce(floor(ud(1)):ceil(ud(2)))./cosd(incidence_array(i)));
end
mean(r_values./div')
A_values = r_values.^2*pi;
A_source = (0.75)^2*pi;
A_n = A_source ./ cosd(incidence_array);
%}
%%
%{
figure(10) % Polar angle difference as function of source area.
%plot(incidence_array,div./2,'linewidth',2,'color','black','Marker','square','LineStyle','none')
hold on
plot(incidence_array,r_values./2,'linewidth',2,'color','black','Marker','square','LineStyle','--')
%plot(incidence_array, ((2./2))./cosd(incidence_array))
plot(incidence_array,0.75./cosd(incidence_array),'linewidth',2,'color','red','Marker','^','LineStyle','--')
grid('on')
xlabel('Incidence Angle $ \theta_i [^\circ]$',fontsize = 24)
ylabel('Spot radius $r_{sc}[\mathrm{mm}]$',fontsize = 24)
legend({'Simulated Value','$r_{sc} / \cos(\theta_i)$'},'Interpreter','latex','location','northwest')
%ylabel('$\mathrm{|\theta_{s,max}-\theta_{s,min}|} [^\circ]$',fontsize = 20)
set(gca,FontSize = 22);
...
avgpol_1 = mean(brdf_M(:,2:91),2);
avgpol_2 = mean(brdf_M(:,91:end),2);
...
xlabel('Polar Angle $\theta_p$ [$^\circ$]',FontSize = 24)
ylabel('Radiant Flux $\Phi_s$[W]',FontSize = 24)
grid('on')
lgd = legend({'$\Phi_{s,Detector}$','$\Phi_{s,calc}$'},'Interpreter','latex','FontSize',20);
lgd.Position = [0.5, 0.5, 0.1, 0.1]; % [left, bottom, width, height]
set(gca,'YScale','log','FontSize',22)
xlim([0,90])

figure(16)
plot(cat(2,-Polar_filter,Polar_filter),cat(1,avgpol_2,avgpol_1),'Color','black','Marker','square','LineStyle','--','LineWidth',2)
...
plot(cat(2,-Polar_filter,Polar_filter),(1/pi)*P_Ref*ones(1,2*numel(Polar_filter)),'Color','green','LineWidth',2)
grid('on')
legend({'$B_{s,\mathrm{mean}}$','$B_{s,\varphi = 0^\circ}$','$B_{s,\varphi = 180^\circ}$','$B_{Lamb} = \frac{1}{\pi} \cdot \Phi_{s,Ref}$'},'Interpreter','latex','FontSize',20)
