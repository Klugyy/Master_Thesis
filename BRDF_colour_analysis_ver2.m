...
white_files =dir(fullfile('..\',append(data_date,'_BRDF_Albert'),'*_SP_COUNTS1_Samp_Ref*.txt')); %dir(fullfile('*_SP_COUNTS_Ref*.txt')); %
...
white_data = white_data([1:5]); %white_data([9:-1:6,1,2:5]); %select correct order of measurements
...
white_rel_sensor_ang(:,2) = white_rel_sensor_ang(:,1);
white_rel_sensor_ang(:,3) = white_rel_sensor_ang(:,1);
white_rel_sensor_ang(:,4) = white_rel_sensor_ang(:,1);
white_rel_sensor_ang(:,5) = white_rel_sensor_ang(:,1);
...
sample_files =dir(fullfile('..\',append(data_date,'_BRDF_Albert'),'*_SP_COUNTS_*_SAng*.txt')); %dir(fullfile('..\*_BRDF_Albert','*_Lenz_*.txt'));
...
var_ang = [-10:1:10;5:1:25;20:1:40;35:1:55;50:1:70]; % ;5:1:25;20:1:40;35:1:55;50:1:70
for idx = 1:size(f_ellipse,2)
    f_ellipse(:,idx) = calcOverlapFactor_v2(ones(21,1).*f_ellipse_angles(idx),var_ang(idx,:)','sensorspot_overlap_small.mat'); %calcOverlapFactor_v2(ones(37,1).*f_ellipse_angles(idx),[-90:5:90]','sensorspot_overlap_small.mat');
end
...
BRDF_saturated = reshape([BRDF_measinfo.Spectrum_Saturated],21,nsamples,[])==1;
